...
10. **Expandable cards.** Clicking a card expands a text-only preview of the note inside the sidebar instead of opening it. The preview leaves out frontmatter, the portrait image (already the card's thumbnail) and the repeated title heading; other images show as 64px cropped squares that open full-screen when clicked, and has a Modify MD button to open the note (plus an inline Edit toggle, see 27).
